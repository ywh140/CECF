...
class BERTSentenceEncoder(nn.Module):
    def __init__(self, config,ckptpath=None):
        ...
        if ckptpath != None:
            ...
            self.bert = BertModel.from_pretrained(config["pretrained_model"],state_dict=ckpt["bert-base"], local_files_only=True)
        else:
            self.bert = BertModel.from_pretrained(config["pretrained_model"], local_files_only=True)
        ...
        #'''
        ###5 no freeze
        self.tokenizer = BertTokenizer.from_pretrained(config["pretrained_model"], local_files_only=True)
